...
const plugin = {
    ...
    onMutation(state, context) {
        ...
        if (!panel) {
            if (!state.missingLogged) {
                Logger.log('⚠ Workflow panel not found for duplicate-to-end');
        ...
        if (!toolsContainer) {
            if (!state.missingLogged) {
                Logger.log('⚠ Tools container not found for duplicate-to-end');
                state.missingLogged = true;
            }
            return;
        }
    ...
    duplicateToolToEnd(card, duplicateBtn) {
        const panel = this.findWorkflowPanel();
        if (!panel) {
            Logger.warn('Workflow panel missing during duplicate-to-end');
            return;
        }
