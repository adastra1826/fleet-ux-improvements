    ...
    onMutation(state, context) {
        ...
        if (!buttonContainer) {
            if (!state.missingLogged) {
                Logger.debug('Guideline Buttons: toolbar container not found (comp-use revision)');
        ...
        if (!wrapper) {
            ...
            Logger.log('✓ Guideline Buttons: wrapper added to top bar (comp-use revision)');
        }
    ...
    findToolbarContainer() {
        ...
        if (!buttonContainer) {
            ...
            const startRecordingBtn = buttons.find(btn => {
                ...
                return text.includes('Start Recording') || text.includes('Resetting');
            });
