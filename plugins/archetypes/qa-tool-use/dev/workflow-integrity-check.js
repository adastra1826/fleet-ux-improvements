...
const plugin = {
    ...
    addToolbarButton(state, context) {
        ...
        if (!center) {
            if (!state.missingLogged) {
                Logger.debug('Action bar center not found for Workflow Integrity Check button');
        ...
        const existing = center.querySelector('[data-fleet-plugin="workflowIntegrityCheck"][data-slot="toolbar-button"]');
        if (existing) {
        ...
        button.innerHTML = '<span class="whitespace-nowrap text-md font-medium">✓ Workflow Integrity Check</span>';
    ...
    getPromptText() {
        // Try the same selector as request-revisions plugin
        ...
        // Fallback: try to find any textarea or div with prompt content
